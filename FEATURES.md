...
## 29. Déploiement Cloud Run : impossible de rejoindre une partie depuis un téléphone (19 août 2026) — ✅ Corrigé

**Rapport de l'utilisateur** : « ppl using pc are joining normally (login that join a game using a code) but ppl using a phone login normally but when trying to join using a code a red error message appear "you must be connected" ». Uniquement sur téléphone ; PC toujours fonctionnel.

**Diagnostic** : le script de déploiement Cloud Run (section précédente) publie `web` et `server` comme deux services Cloud Run séparés, donc deux domaines `*.run.app` différents. Le cookie de session posé par `/api/auth/*` (voir `apps/server/src/auth/cookies.ts`) devient alors, du point de vue du navigateur, un cookie tiers (« third-party ») envoyé depuis le domaine `web` vers le domaine `server` — même avec `SameSite=None; Secure` correctement configurés (vérifié dans `deploy-cloudrun.ps1` et `cookies.ts`). Safari iOS (Intelligent Tracking Prevention) bloque ou purge ce genre de cookie tiers de façon beaucoup plus agressive que les navigateurs desktop, ce qui explique exactement le symptôme : le login (POST vers `/api/auth/*`, dans un contexte encore « premier accès ») réussit visiblement, mais la connexion Socket.IO faite juste après pour rejoindre une partie (`PLAYER_JOIN`) ne porte plus le cookie — le serveur voit une requête sans session et répond « Vous devez être connecté pour rejoindre une partie. ». Ce problème n'existait pas avant (setup Tailscale/nginx auto-hébergé) parce que tout était déjà servi depuis une seule origine — voir `proxy/nginx.conf`.

**Corrigé** : un troisième service Cloud Run, `loupgarou-proxy` (nginx, `proxy/nginx.cloudrun.conf.template` + `proxy/Dockerfile.cloudrun`), reproduit exactement le rôle que joue déjà `proxy/nginx.conf` en local — un seul point d'entrée public qui redirige `/socket.io/`, `/api/`, `/health` vers `server` et tout le reste vers `web`, remettant les deux services derrière une seule origine du point de vue du navigateur. Comme les URL Cloud Run (`*.run.app`) ne sont connues qu'une fois chaque service déployé, la config nginx utilise des variables d'environnement (`SERVER_UPSTREAM`, `WEB_UPSTREAM`) substituées au démarrage du conteneur via le mécanisme `envsubst` déjà intégré à l'image officielle `nginx` — pas besoin de reconstruire l'image pour changer une URL amont, un simple `gcloud run services update --update-env-vars` suffit. Le `proxy_pass` utilise des variables nginx (pas des valeurs littérales) associées à une directive `resolver`, pour forcer une résolution DNS à chaque requête plutôt qu'une résolution figée au démarrage — nécessaire puisque les URL Cloud Run ne pointent pas vers une IP fixe.

`deploy-cloudrun.ps1` a été réordonné en conséquence : (5) le proxy est buildé et déployé une première fois avec `WEB_UPSTREAM` en placeholder, juste pour obtenir son URL stable ; (6) `web` est ensuite buildé avec `NEXT_PUBLIC_SERVER_URL` pointant vers **l'URL du proxy** (et non plus celle du serveur directement) ; (7) `WEB_UPSTREAM` du proxy et `CORS_ORIGIN` du serveur sont enfin mis à jour vers les vraies URL. Aucun changement de code applicatif n'a été nécessaire côté `apps/web` — `lib/socket.ts` et `lib/api.ts` utilisaient déjà `NEXT_PUBLIC_SERVER_URL` comme unique point de configuration, il suffisait qu'il pointe vers la bonne origine.

**Important pour la suite** : c'est désormais l'URL du **proxy** (`loupgarou-proxy-*.run.app`) qu'il faut partager avec les joueurs, jamais celle de `web` directement — le script l'affiche en évidence à la fin. `AUTH_COOKIE_SAMESITE=none` n'a pas été repassé à `lax` (ça reste inoffensif même en same-origin) pour limiter le risque de ce correctif urgent ; c'est un nettoyage facultatif pour plus tard, pas un problème fonctionnel.

**Tests** : correctif d'infrastructure pur (aucun code applicatif modifié) — pas de nouveau test unitaire ; vérifié par relecture de `apps/web/src/lib/socket.ts` et `apps/web/src/lib/api.ts` (confirmant que les deux utilisent déjà `NEXT_PUBLIC_SERVER_URL` comme seule source de vérité) et par une vérification manuelle d'équilibrage des accolades/guillemets du script PowerShell modifié (même classe de bug qui avait cassé le script la première fois — voir section précédente). Suite de tests existante (401 tests) inchangée et toujours verte, aucun fichier testé n'a été touché.
