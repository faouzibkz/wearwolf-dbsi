...
- **18 août 2026 (§27) — pause automatique quand le joueur "sur qui on attend" se déconnecte.** `GameEngine.getCurrentActionRequiredPlayerIds()` (nouveau, juste après `hasPendingBlockers()`) répond à une seule question pour n'importe quelle phase : quels `playerId` la partie attend-elle littéralement en ce moment ? Priorité systématique aux blocages en attente (tireur Chasseur / Chef mourant en attente de successeur, toujours en premier, même en pleine `DAY_VOTE` ou `NIGHT`), puis un `switch` par phase qui délègue aux getters "current speaker/voter" déjà existants — `getNightPrompts()` en particulier est déjà à la fois "seulement ceux qui n'ont pas encore agi" ET, en mode SEQUENTIAL, "seulement le rôle de l'étape en cours", donc réutilisable tel quel sans aucune branche NIGHT-mode-spécifique ici. `pauseForDisconnect(playerId)`/`resumeFromDisconnect(playerId)` réutilisent intégralement le mécanisme `pause()`/`resume()`/`pausedRemainingMs` déjà construit pour le bouton pause manuel de l'admin — **aucune modification n'a été nécessaire dans `timers.ts`** : `schedulePhaseTimer` et `schedulePendingBlockerTimer` y respectaient déjà `engine.getPublicState().paused` avant d'armer leur `setTimeout`, donc geler `paused` suffit à geler tous les délais existants, quelle que soit la phase. Piège évité explicitement : `pauseForDisconnect()` ne prend JAMAIS la main si `state.paused` est déjà vrai pour une autre raison (pause manuelle admin) — sinon `resumeFromDisconnect()` risquerait de lever une pause que l'admin avait posée exprès dès que le mauvais joueur se reconnecte. `resume()` (donc `ADMIN_RESUME`) efface systématiquement `disconnectPausedPlayerId`, ce qui rend cette interaction sûre dans les deux sens sans code supplémentaire côté handler. Câblage serveur minimal : un appel dans le handler `disconnect` (`handlers.ts`) et un dans `PLAYER_JOIN`/`PLAYER_RECONNECT`, tous les trois juste après leur `engine.setConnected(...)` existant.
- **19 août 2026 (§29) — déploiement Cloud Run : `web` et `server` sur deux domaines `*.run.app` séparés rend le cookie de session tiers du point de vue du navigateur, bloqué par Safari iOS (ITP) mais toléré par les navigateurs desktop.** Symptôme réel : login visiblement réussi sur téléphone, mais rejoindre une partie via code renvoie « vous devez être connecté » (la connexion Socket.IO qui suit ne porte plus le cookie). PC toujours fonctionnel — desktop est nettement plus tolérant sur les cookies tiers que Safari mobile. Ce problème n'existait pas avec le setup Tailscale/nginx auto-hébergé (une seule origine dès le départ, voir §22 juste en dessous). Corrigé en réappliquant exactement le même principe côté Cloud Run : un troisième service, `loupgarou-proxy` (nginx, `proxy/nginx.cloudrun.conf.template` + `proxy/Dockerfile.cloudrun`), remet `web` et `server` derrière une seule origine publique. Comme les URL `*.run.app` de `web` et `server` ne sont connues qu'après leur propre déploiement (contrairement au setup docker-compose où les noms de service sont fixes), la config nginx utilise des variables d'environnement (`SERVER_UPSTREAM`/`WEB_UPSTREAM`) substituées au démarrage via le mécanisme `envsubst` déjà intégré à l'image officielle `nginx:1.27-alpine` (fichier posé dans `/etc/nginx/templates/*.template`, aucun entrypoint personnalisé requis) — un simple `gcloud run services update --update-env-vars` suffit ensuite pour changer une URL amont, sans reconstruire l'image. `proxy_pass` utilise des variables nginx associées à une directive `resolver` (pas des valeurs littérales) pour forcer une résolution DNS à chaque requête plutôt qu'au démarrage, puisque les URL Cloud Run ne correspondent pas à une IP fixe ; `proxy_ssl_server_name on` + `Host: $proxy_host` sont nécessaires en plus (absents de `proxy/nginx.conf`, la version docker-compose) car ici l'amont est en HTTPS et Cloud Run route par SNI/Host — envoyer le Host du CLIENT plutôt que celui de l'amont ferait échouer la requête. `deploy-cloudrun.ps1` déploie le proxy une première fois avec `WEB_UPSTREAM` en placeholder (juste pour obtenir son URL stable, avant même que `web` existe), buildé `web` avec `NEXT_PUBLIC_SERVER_URL` pointant vers cette URL de proxy (pas celle du serveur), puis corrige `WEB_UPSTREAM`/`CORS_ORIGIN` une fois tout déployé — aucun changement de code applicatif requis, `lib/socket.ts`/`lib/api.ts` utilisaient déjà `NEXT_PUBLIC_SERVER_URL` comme unique source de vérité.
- **19 août 2026 (§28) — `broadcastGameState` diffuse maintenant DEUX versions différentes de l'état, pas une seule.** Jusqu'ici `engine.getPublicState()` alimentait à la fois `GAME_STATE` (toute la salle) et `ADMIN_STATE` (l'hôte seul) — un vrai bug de confidentialité l'a mis en évidence : le statut `isConnected` réel de chaque joueur (et, depuis hier soir, `disconnectPausedPlayerId`) atteignait TOUS les joueurs, révélant indirectement mais fiablement qui agit en ce moment (une déconnexion visible coïncide presque toujours avec le tour du joueur concerné). `broadcast.ts` calcule maintenant `fullState = engine.getPublicState()` une seule fois, l'envoie tel quel à `ADMIN_STATE` (l'admin a un besoin légitime de savoir qui est réellement déconnecté), et diffuse à la salle `sanitizeForRoom(fullState)` — une copie superficielle qui force `isConnected: true` pour tout joueur VIVANT (les morts/spectateurs ne sont pas concernés, cohérent avec l'Afterlife) et `disconnectPausedPlayerId: null`. Le nouveau champ `GameStatePublic.isPausedForDisconnect` (booléen, calculé côté moteur via `isPausedForDisconnect()`, jamais filtré puisqu'il ne porte aucune identité) est ce que l'UI joueur doit utiliser pour savoir "une pause est en cours", jamais `disconnectPausedPlayerId` directement. Ce pattern (une fonction `sanitizeForXxx` dédiée dans `broadcast.ts`, appelée juste avant l'émission à la salle, jamais avant l'émission admin) est le bon gabarit à réutiliser pour toute future donnée qui doit rester visible pour l'admin mais pas pour les joueurs. Côté web, `PlayerList.tsx` a gagné une prop `showConnectionStatus` (fausse par défaut, vraie uniquement dans `admin/[code]/page.tsx`) plutôt que de supprimer purement et simplement l'icône ⚡ — supprimer l'icône entièrement aurait aussi aveuglé l'admin, qui a besoin de cette info pour gérer la partie.
