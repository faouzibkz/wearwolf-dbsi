...
- **Test connu instable/en échec : `apps/server/src/socket/idleCleanup.test.ts` → "any successful gameRegistry.requireGame() call resets the idle clock"** (découvert le 17 août 2026, en faisant tourner la suite complète pour la première fois avec la méthode ci-dessus). Échoue de façon reproductible dans cet environnement Docker précis — pas un flake aléatoire, mais confirmé **préexistant et sans rapport** avec quoi que ce soit fait ce jour-là (`git diff` vide sur ce fichier et sur `idleCleanup.ts` contre `85f3855`, un commit d'avant toute modification de la session). Cause probable : le test utilise `Date.now()` réel (pas de fake timers) avec une marge de tolérance qui semble trop courte pour la latence I/O de ce `docker compose run` précis. **Ne pas supposer qu'un futur changement l'a cassé** sans vérifier `git diff` sur ces deux fichiers en premier — et il reste encore à corriger, pas fait ici (hors scope du travail en cours à ce moment-là).
